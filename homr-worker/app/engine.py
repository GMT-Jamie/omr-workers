...
（目錄模式**會把每張圖處理兩遍** —— 每張圖 10 次 tromr 推論而不是 5 次 —— 所以要
除以 2 才是同一個工作量。）

也就是說行程啟動 + ONNX session 建立大約吃掉單頁的 28%。那個比例落在「值得換」的
門檻上，而不是「明顯不值得」。

**注意 bench.sh 的 `total - inference` 差不是這個數字**：那個差只有 10 ms 級
（FastAPI 的請求開銷），因為 `inference` 量的是整個 subprocess，session 建立就在
它裡面。

TODO(verify): 在容器裡重量一次（`docker compose exec` 跑目錄模式）。若 28% 站得住，
改成 import `homr.main.process_image` 並在 lifespan 預熱 —— 代價是綁一個沒有穩定性
承諾的內部函式，而且 `ready` 的語意會從「引擎可執行」變成「模型已載入」（那才是
spec §4 本來要的）。
