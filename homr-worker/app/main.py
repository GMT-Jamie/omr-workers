...
對外契約見 README 的〈HTTP 介面〉（註解裡的 `spec §N` 指呼叫端專案裡的內部契約
文件，內容與 README 相同）。這個服務是**啞巴工人**：收圖、回 MusicXML，不含任何
呼叫端的業務邏輯。排隊、配額、路由、MusicXML 的後處理全部歸呼叫端。

## 單工是刻意的

容量 1 的 semaphore，滿載時**立即回 429，不排隊**。理由：單頁推論是 20 秒級的
CPU 佔用（實測 24 秒），排隊只是把「現在很忙」延後成「等了兩分鐘還是很忙」，而
呼叫端本來就是決定排隊策略的那一層。它看得到 429 才能決定要不要重試。
